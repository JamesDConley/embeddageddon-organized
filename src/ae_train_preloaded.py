...
"""
Train Embeddageddon - Autoencoder using Preloaded Data in RAM

This script trains the autoencoder with all data preloaded into RAM for maximum performance.
Eliminates all disk I/O during training for best GPU utilization.

Usage:
    1. First run: python preprocess_embeddings.py --embedding_dir <dir> --output_dir <preprocessed_dir>
    2. Then run: python ae_train_preloaded.py --preprocessed_dir <preprocessed_dir>

Note: Requires sufficient RAM to hold the entire dataset (typically 2-10 GB depending on dataset size)
"""
...
from autoencoder.preloaded_dataset import PreloadedEmbeddingDataset
...
def save_training_config(output_dir: Path, args: argparse.Namespace, dataset: PreloadedEmbeddingDataset):
    """
    ...
    """
    config = {
        "training_type": "preloaded_train_with_random_subnetwork",
        "training_args": {
...
def train_autoencoder(dataset: PreloadedEmbeddingDataset,
                     output_dir: Path,
                     num_epochs: int = 100,
                     batch_size: int = 32,
    ...
    """
    # Create data loader
    # All data is preloaded in RAM, so we can use num_workers=0 for simplicity
    # or enable workers for async batching if desired
    dataloader = DataLoader(
        dataset,
        batch_size=batch_size,
        shuffle=True,
        num_workers=num_workers,  # Can use 0 since data is already in RAM
        pin_memory=True,          # Safe to enable since data is in RAM, not being created on-the-fly
    ...
    print(f"Data loading: {num_workers} workers (preloaded in RAM)")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train Embeddageddon Autoencoder (Preloaded)")
    ...
    parser.add_argument("--num_workers", type=int, default=0,
                       help="Number of worker processes for data loading (default: 0 since data is in RAM)")
    ...
    if args.output_dir:
        ...
        output_dir = create_timestamped_output_dir("train_run_preloaded")

    print("=" * 60)
    print("EMBEDDAGEDDON TRAINING (PRELOADED)")
    print("=" * 60)
    ...
    try:
        # Load preprocessed dataset (preloads all into RAM)
        print("Loading dataset into RAM...")
        dataset = PreloadedEmbeddingDataset(args.preprocessed_dir)
